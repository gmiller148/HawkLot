...
spotInfo.update = function (properties,addon) {
  occupiedAddition = ' ';
  if (typeof properties !== 'undefined' && !properties.occupied) {
    occupiedAddition = 'not ';
  }
	this._div.innerHTML = (properties ?
		'<h4>This is parking spot number ' + properties.id + '.</h4>' +
    '<p>The spot is ' + occupiedAddition + 'occupied.<br><div align="center"><button class="btn btn-primary" onclick="changeStatus('+ properties.id + ',' + properties.occupied + ')">Rent Me</button></div>'
		: '') + (addon ? addon : '');
};
